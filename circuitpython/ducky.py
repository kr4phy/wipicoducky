...
def runScript(script_string):
    """
    Execute a ducky script from a string.
    
    Args:
        script_string: Ducky script string to execute
    """
    global defaultDelay
    ...
    try:
        while restart:
            restart = False
            # 문자열을 라인별로 분리
            script_lines = iter(script_string.strip().split('\n'))
            previousLine = ""
            for line in script_lines:
                print(f"runScript: {line}")
                if(line[0:6] == "REPEAT"):
